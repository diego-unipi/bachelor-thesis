...
\begin{remark}[On the axioms used in the proof of Russell's paradox]
    ...
    Working by contradiction, all the other axioms follow form $V \in \mathsf{Set}$ + separation, se the statement of the theorem is technically accurate.
\end{remark}

\subsection*{Halting problem}
Let's move to the category of total recursive functions, $\mathsf{Rec_{tot}}$, whose objects are: $\{*\} = \NN^0$, $\NN^1$, $\NN^2$, $\ldots$, and whose morphisms are total recursive functions between them.

\begin{definition}
    A \emph{total recursive function} is a function $f : \NN^n \to \NN_\bot^m$ such that $f(\vec x) \ne \bot$ for every $\vec x \in \NN^n$, where $\NN_\bot := \NN \cup \{\bot\}$, and $\bot$ is a special symbol denoting undefinedness.
\end{definition}

\begin{notation}[Universal function]
    We denote by $u_1(-,-)$ the universal function, see [REFERENCE], of arity 1.
\end{notation}
...
\begin{definition}[$H_1$]
    We call $H_1 := \{(e,x) \in \NN^2 : u_1(e,x) = 0\}$ the \emph{halting set}.
\end{definition}
...
\begin{proof}
    Let's assume, by contradiction, that $H_1$ is decidable, so there exists a total recursive function $f : \NN^2 \to \NN_\bot$ such that
    \[ H_1 = \{(e,x) \in \NN^2 : f(e,x) = 0\}. \]
    We define the total recursive function $g : \NN \to \NN_\bot$ a
\end{proof}
