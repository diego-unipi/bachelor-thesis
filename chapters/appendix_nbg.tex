...
Throughout the exposition, we have been working in the Von Neumann--Bernays--Gödel ($\mathsf{NBG}$) set theory, just like \cite{LambekScott1994} does in Part I.
...
\section{Axioms}
We use the following convenient schematic presentation of $\mathsf{NBG}$. It keeps the $\mathsf{ZFC}$ axioms explicit and adds the specifics of $\mathsf{NBG}$.
...
\begin{definition}[Small calculus]
    A typed $\lambda$-calculus $\mathcal L$ is \emph{small} if both $\mathsf{Ty}(\mathcal L)$ and $\mathsf{Term}(\mathcal L)$ are sets in the sense of $\mathsf{NBG}$.
\end{definition}
...
products and, when required, exponentials and the chosen WNNO. Local smallness makes its collection of arrows a set. Lawvere's theorem itself has no size assumption.

Finally, the ordinary axiom of choice suffices for non-unique iterator choices in a small category; global choice, instead, suffices to make such choices uniformly over a proper class of objects,
