\chapter{NBG set theory}
During the whole exposition, we have been working in the Neumann--Bernays--Gödel ($\mathsf{NBG}$) set theory, just like \cite{LambekScott1994} does in Part I.

Let's us recall the main characteristics of the $\mathsf{NBG}$ set theory and its conservativity result over $\mathsf{ZFC}$.

\section{Sets and classes}
The language of $\mathsf{NBG}$ has two sorts of variables: lower-case letters $x,y,z,\ldots$ symbolizing sets, while upper-case letters $X,Y,Z,\ldots$ denotes the classes; and a binary relation symbol $\in$ for membership.
...
Global choice necessity will be discussed below in the section that regards the sizes of the categories we used.

\section{The axioms}
We use the following convenient schematic presentation of $\mathsf{NBG}$. It keeps the $\mathsf{ZFC}$ axioms explicit and add the specifics of $\mathsf{NBG}$.
...
\begin{itemize}
    ...
    If $a$ is a set and $\varphi(x)$ is a formula, then $\{x\in a:\varphi(x)\}$ is a set:
    ...
    Given a set $a$, there exists a set whose elements are the elements of the elements of $a$:
    ...
    \item[(\textnormal{NBG12})] \textbf{Axiom schema of elementary class comprehension.}\par
    If $\varphi(x,a_1,\ldots,a_n,A_1,\ldots,A_m)$ is formula with one free variable $x$, then there exists the class of all sets which satisfy $\varphi$:
    \[\exists X\;\forall x\;\left(x\in X\leftrightarrow\varphi(x,a_1,\ldots,a_n,A_1,\ldots,A_m)\right).\]
...
These axioms follow the exposition given in [REFERENCE].
...
\begin{theorem}[Conservativity of $\mathsf{NBG}$ over $\mathsf{ZFC}$]\label{thm:nbg-conservativity}
    For every sentence $\varphi$ in the language of set theory, hence containing no class variables,
    \[\mathsf{NBG}\vdash\varphi\quad\Longleftrightarrow\quad\mathsf{ZFC}\vdash\varphi.\]
    In other words, every theorem about sets which can be proved using classes can already be proved in $\mathsf{ZFC}$.
\end{theorem}

See [REFERENCE] for a proof of this theorem.

\section{Sizes of the categories used}

\begin{definition}[Small category]
    A category $\mathcal C$ is \emph{small} if both $\ob(\mathcal C)$ and $\mor(\mathcal C)$ are sets.
\end{definition}

\begin{definition}[Locally small category]
    \leavevmode\newline
    A category $\mathcal C$ is \emph{locally small} if $\Hom_{\mathcal C}(A,B)$ is a set for every pair of objects $A,B$.
\end{definition}

\begin{remark}
    Every small category is locally small, but the converse need not hold.
\end{remark}
...
\begin{definition}[Small calculus]
    A calculus $\mathcal L$ is \emph{small} if both $\ob(\mathcal L)$ and $\mor(\mathcal L)$ are sets.
\end{definition}
...
The resulting categories of structures are large but locally small. Under this convention, polynomial categories, $\mathbf L(\mathcal C)$ and $\mathbf C(\mathcal L)$ preserve smallness, so the three correspondences are ordinary equivalences in $\mathsf{NBG}$.
Local smallness alone would not suffice: a category with a proper class of objects cannot itself be an object of another category at the same foundational level.

The ambient categories in the fixed-point applications need not be small. If such a category is large but locally small, the CHL argument can be carried out in the small full subcategory generated by the objects involved and closed under the terminal object, products and, when required, exponentials and the chosen WNNO. Local smallness makes its collection of arrows a set. Lawvere's theorem itself has no size assumption. Finally, ordinary choice suffices for non-unique iterator choices in a small category; global choice is needed only to make them uniformly over a proper class of objects, while for an NNO uniqueness removes the need for choice.
