...
\begin{notation}
    $S[a/x]$ can regarded as the process of \emph{substituting} the arrow $a$ for the indeterminate arrow $x$; therefore, we will use the notation:
...
We'll try to do something similar for the arrows of $\mathcal C[x]$ and obtain \emph{Lambek's normal form}. In order to do this, we first need to introduce a couple
of results, each a stronger version of the previous one. These results go under the name of \emph{deduction theorems} or, more broadly, \emph{functional completeness} theorems.
...
\begin{definition}
    ...
    where $h \in \mor(\mathcal P)$ and $A \in \ob(\mathcal P)$. In particular, the identity, terminal, projection and evaluation arrows $1_B$, $!_B$, $\pi_{B,C}$, $\pi_{B,C}'$ and $\textnormal{ev}_{B,C}$ of $\mathcal P$ are the same as those of $\mathcal P\langle x \rangle$, since they are included in the $h$ above.
...
\begin{corollary}[Lambek's normal form]\label{cor:lambeks-normal-form}
    ...
    Moreover, there's a unique $h : 1 \to C^A$ such that $\textnormal{ev}_{C,A} \circ \langle h,x \rangle \underset{x}{=} \varphi(x)$. Equivalently, $\Hom_{\mathcal C[x]}(1,C) \cong \Hom_{\mathcal C}(1,C^A)$.
\end{corollary}
