...
\begin{definition}[Graph]\label{idx:graph}
...
\begin{notation}
    We will denote by $\Hom_{\mathcal G}(A,B)$, omitting the subscript when it is clear from the context, the class of arrows $f$ such that $s(f)=A$ and $t(f)=B$.\label{idx:hom}
\end{notation}
...
\begin{definition}[Conjunction calculus]\label{idx:conjunction}
...
\begin{definition}[Positive intuitionistic calculus]\label{idx:implication}
...
\begin{example}
    Given an arrow $f : A \to B$ we can define the arrow $\ulcorner f \urcorner$ (often called \emph{Lawvere's name} of $f$) as\label{idx:lawvere-name}
    \[ \frac{A \xrightarrow{f} B}{\top \xrightarrow{\ulcorner f \urcorner} B \Leftarrow A}, \]
    which is just $\ulcorner f \urcorner := (f \circ \pi_{\top,A}')^*$. Similarly, given $g : \top \to B \Leftarrow A$, we can define
    \[ \frac{\top \xrightarrow{g} B \Leftarrow A}{A \xrightarrow{g^\sharp} B} \]
    with $g^\sharp := \ev_{B,A} \circ \langle g \circ !_A,1_A\rangle$.\label{idx:global-evaluation}
\end{example}
...
Cartesian closed categories, presented equationally, first appeared in work by Lambek \cite{Lambek1968,Lambek1969,Lambek1972}, who regarded them as certain deductive systems, with an equivalence relation between proofs.
Independently, Prawitz \cite{Prawitz1971} also introduced an equivalence relation between proofs, which was later shown by Mann \cite{Mann1975} and Seely \cite{Seely1977} to be equivalent to Lambek's.
...
\begin{notation}[Terminal object and product]\label{idx:products}
...
\begin{notation}[Product of arrows]\label{idx:product-arrows}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \[
        \makebox[\linewidth][c]{$\begin{aligned}
...
\begin{remark}[Equivalent definition of cartesian closed category]\label{idx:currying}
...
\begin{remark}
    Throughout the whole exposition, whenever $\cong$ is used between hom-sets, it denotes only a bijection between the underlying sets. We will sometimes use $\overset{\textnormal{1:1}}{\longleftrightarrow}$ instead, with the same meaning.
\end{remark}
...
\begin{notation}[Exponential object]\label{idx:exponential}
...
\begin{definition}[Positive intuitionistic calculus freely generated by a graph]\label{idx:free-calculus}
...
\begin{definition}[Cartesian closed category generated by a graph]\label{idx:free-ccc}
...
\begin{example}[Smaller congruence relation than parallelism]
    Let us consider the following graph $\mathcal G$:
    % https://q.uiver.app/#q=WzAsMixbMCwwLCJBIl0sWzIsMCwiQiJdLFswLDEsImYiLDAseyJjdXJ2ZSI6LTN9XSxbMCwxLCJoIiwyLHsiY3VydmUiOjN9XSxbMCwxLCJnIl1d
    \[\begin{tikzcd}
        A && B
    ...
    \end{tikzcd}.\]
    Let $\sim$ be the smallest equivalence relation on parallel arrows containing the category, cartesian and cartesian closed axioms, compatible with composition, pairing and currying, and satisfying $f \sim g$.
    Interpreting $A$ as $\{\ast\}$, $B$ as $\{0,1\}$, $f,g$ as the constant function with value $0$ and $h$ as the constant function with value $1$ in the category of sets shows that $f \not\sim h$.
    Thus $\sim$ is strictly smaller than the relation of having the same source and target: its quotient has at least two arrows from $A$ to $B$, whereas the latter quotient has only one.
\end{example}
...
\begin{notation}[Polynomial arrows]\label{idx:polynomial-arrows}
...
\begin{definition}[Polynomial category -- v.2]
    ...
    \begin{itemize}
        ...
        is a (cartesian, cartesian closed) functor. We choose the smallest equivalence relation on parallel arrows satisfying the following properties, and we denote it by $\underset{x}{=}$:\label{idx:polynomial-equality}
        \begin{itemize}[label=$\ast$]
...
\begin{notation}[Canonical functor $\mathcal C \to \mathcal C{[x]}$]\label{idx:polynomial-inclusion}
...
\begin{proof}
    This follows by applying the universal property with $F = 1_\mathcal C$.
\end{proof}

\begin{notation}
    $S[a/x]$ can be regarded as the process of \emph{substituting} the arrow $a$ for the indeterminate arrow $x$; therefore, we will use the notation:\label{idx:substitution}
...
The functional completeness of cartesian closed categories was proved by Lambek in \cite{Lambek1972}, with extra assumptions w.r.t. the ones we have here, and then without them in \cite{Lambek1974}.
...
\begin{notation}
    From now on we will denote by $\kappa_{x \in A}\varphi(x)$ the arrow in $\mathcal P$ associated to $\varphi(x)$ by the deduction theorem above.\label{idx:deduction}
        ...
        \begin{itemize}
            ...
            \begin{align*}
                \kappa_{x \in A}(\psi(x) \circ \chi(x)) \circ \langle x \circ !_B, 1_B\rangle &\underset{x}{=} \kappa_{x \in A}(\psi(x)) \circ \langle \pi_{A,B}, \kappa_{x \in A}(\chi(x))\rangle \circ \langle x \circ !_B, 1_B\rangle \\
                                                                                              ...
                                                                                              &\underset{x}{=} \kappa_{x \in A}(\psi(x)) \circ \langle x \circ !_D, 1_D\rangle \circ \chi(x) \quad(\textnormal{by $\spadesuit$})\\
...
\begin{definition}[Terminal category]
    We say that a category $\mathcal C$ is \emph{terminal} if it has exactly one object and one arrow. In this case we denote it by $\mathbf 1$.
\end{definition}
...
\begin{example}[Collapse of $\mathsf{Set}{[x]}$, {\cite[Part~I, \S5]{LambekScott1994}}]\label{ex:collapse-of-set-x}
    ...
    By (CC2b), $\pi_{\varnothing,B}' \circ s \underset{x}{=} 1_B$. Moreover, since $\varnothing \times B = \varnothing$ and in $\mathsf{Set}$ there is only the empty function from $\varnothing$ to any set, the arrows
    \[ f \circ \pi_{\varnothing,B}', g \circ \pi_{\varnothing,B}' : \varnothing \to C \]
    are equal. Therefore, in $\mathsf{Set}[x]$
    \begin{align*}
        f &\underset{x}{=} f \circ 1_B \\
          ...
          &\underset{x}{=} g \circ 1_B \underset{x}{=} g.
    \end{align*}
    Hence the canonical functor $\iota_x : \mathsf{Set} \to \mathsf{Set}[x]$ is not faithful, since it identifies all parallel arrows of $\mathsf{Set}$.
    ...
    The last set has exactly one element, so there is exactly one arrow between any ordered pair of objects in $\mathsf{Set}[x]$.
    Thus all its objects are isomorphic and $\mathsf{Set}[x]$ is equivalent to the terminal category. Logically, adjoining $x : 1 \to \varnothing$ amounts to assuming a proof of falsehood, so this collapse is the categorical counterpart of the principle of explosion (ex falso quodlibet).
\end{example}
