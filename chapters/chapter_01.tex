...
\begin{remark}[Category freely generated by a deductive system]\label{rem:category-generated-by-deductive-system}
    ...
    i.e.,\ the smallest equivalence relation such that the equations above hold (it is well-defined, for example, since there exists the equivalence relation of having the same source and target, which is compatible with composition and contains the identity and associativity laws). 
\end{remark}
...
\begin{definition}[Cartesian closed category generated by a graph]
    ...
    is the quotient of $\mathcal P(\mathcal G)$ by the smallest equivalence relation on proofs that contains the category, cartesian and cartesian closed axioms and is compatible with composition, pairing and currying.
\end{definition}
...
\begin{definition}[Polynomial category - v.2]
    ...
    \begin{itemize}
        ...
        is a (cartesian, cartesian closed) functor. We choose the smallest equivalence relation with these properties, and we denote it by $\underset{x}{=}$. In the case of a cartesian closed category, we require $\underset{x}{=}$ to satisfy the following properties:
        \begin{itemize}[label=$\ast$]
