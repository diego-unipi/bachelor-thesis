...
\begin{remark}[Derived rules of inference]
    ...
    \end{itemize}
    Both of this can be derived from the original rules, for example taking $\eta_{C,B} := 1_{C \land B}^*$ and $g \Leftarrow 1_B := (g \circ \ev_{D,B})^*$. It can also be shown that (Impl 1') and (Impl 2') implies (Impl 2),
...
\begin{example}[Co-pairing arrow]
    ...
    This inference rule is the real counterpart of the elimination rule for disjunction in $\mathsf{ND}$ (the Natural Deduction system). 
\end{example}
...
\begin{definition}[Freely generated positive intuitionistic calculus by a graph]
    Given a graph $\mathcal G$, the \textit{freely generated positive intuitionistic propositional calculus} generated by $\mathcal G$, denoted with $\mathcal P(\mathcal G)$,
...
\begin{notation}
    We denote with $\mathsf{CCC}$ the category of cartesian closed categories (whose morphisms are cartesian closed functors) and with $\mathsf{Grph}$ the category of graphs (whose morphisms are maps that commute with the functions $s$ and $t$).
...
\begin{definition}[Canonical inclusion into the free CCC underlying graph]
    ...
    each arrow $f : X \to Y$ of $\mathcal G$ is sent to its equivalence class $[f]$ in $\mathcal U(\mathcal F(\mathcal G))$, i.e. the same 
    equivalence class of $f$ in $\mathcal F(\mathcal G)$, we're just forgetting the additional structure properties given by the cartesian closedness.
\end{definition}
...
\begin{proof}
    ...
    This definition of $\overline F$ can be recursively extended on the objects of $\mathcal P(\mathcal G)$ by imposing the cartesian closed structure, therefore $\overline F$ is uniquely defined on the objects of $\mathcal F(\mathcal G)$\footnote{To be completely rigorous, we should invoke the principles of recursion on the grammar, to define $\overline F$, and induction on the grammar, to prove its uniqueness.}.\par
...
The previous definition it's not precise but it gives us the general idea, it can be made precise by following a similar approach to the one used to define the cartesian closed category freely generated by a graph.
...
\begin{definition}[Polynomial category - v.2]
    ...
    in two steps:
    \begin{itemize}
        \item first we define the positive intuitionistic propositional calculus, this time, since $\mathcal C$ is already a cartesian closed category, we can directly take its objects $\ob(\mathcal C)$ as the objects of the calculus; for the arrows,
        we take the ones generated by the following grammar:
        \[ f,g ::= h \mid 1_A \mid !_A \mid f \circ g \mid \pi_{A,B} \mid \pi_{A,B}' \mid \langle f,g\rangle \mid f^* \mid \ev_{A,B} \mid x, \]
        where $A, B \in \ob(\mathcal C)$, $h \in \mor(\mathcal C)$ and $x : A_0 \to A$ is the indeterminate arrow;
...
\begin{remark}[Well-posedness of the definition v.2]
    ...
    One can easily check that this equivalence relation contains also the remaining properties above, therefore the quotient is a (cartesian, cartesian closed) category and the immersion $\iota_x : \mathcal C \rightarrow \mathcal C[x]$ is a (cartesian, cartesian closed) functor,
    so the definition above is well-posed.
\end{remark}

\begin{notation}[Canonical functor $\mathcal C \to \mathcal C{[x]}$]
    From now on we will denote with $\iota_x : \mathcal C \rightarrow \mathcal C[x]$ the immersion of $\mathcal C$ into $\mathcal C[x]$, which is a (cartesian, cartesian closed) functor.
...
\begin{theorem}[Universal property of polynomial categories]
    Given a (cartesian, cartesian closed) category $\mathcal C$, two objects of such category $A_0$ and $A$, an indeterminate arrow $x : A_0 \to A$, a (cartesian, cartesian closed) category $\mathcal D$, a (cartesian, cartesian closed) functor $F : \mathcal C \to \mathcal D$ and an arrow $a : F(A_0) \to F(A)$ in $\mathcal D$,
...
Such normal form wants to be unique and follows the same idea behind the normal form of polynomials in $R[x]$, that is, for a commutative unital ring $R$,
every polynomial can be uniquely expressed in the form $a_n x^n + a_{n-1} x^{n-1} + \ldots + a_1 x + a_0$, with $a_i \in R$.\par
We'll try do a similar thing for the arrows of $\mathcal C[x]$ and to get the \textit{Lambek's normal form}. In order to do this we first need to introduce a couple 
of results, each a stronger version of the previous one, they go under the name of \textit{deduction theorem} or, more broadly, \textit{functional completeness}.
