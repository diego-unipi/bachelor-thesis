...
\begin{definition}[Polynomial category - v.2]
    ...
    \begin{itemize}
        ...
        is a (cartesian, cartesian closed) functor. We choose the smallest equivalence relation on parallel arrows with these properties, and we denote it by $\underset{x}{=}$. In the case of a cartesian closed category, we require $\underset{x}{=}$ to satisfy the following properties:
        \begin{itemize}[label=$\ast$]
        ...
        \end{itemize}
        The fact that $!_A^{\mathcal C} \underset{x}{=} !_A^{\mathcal C[x]}$ follows from (CC1) in $\mathcal C[x]$; the fact that $\pi_{A,B}^{\mathcal C} \underset{x}{=} \pi_{A,B}^{\mathcal C[x]}$ follows from (CC2a) + the fact that $1_A^{\mathcal C} \underset{x}{=} 1_A^{\mathcal C[x]}$ and the preservation of pairing:
        ...
        now composing with $\pi_{A,B}^{\mathcal C[x]}$ we get $\iota_x(\pi_{A,B}^{\mathcal C}) \underset{x}{=} \pi_{A,B}^{\mathcal C[x]}$. Exactly the same argument applies to $(\pi_{A,B}')^{\mathcal C}$. Lastly, the fact that $\ev_{A,B}^{\mathcal C} \underset{x}{=} \ev_{A,B}^{\mathcal C[x]}$
        ...
        this identity follows solely from (CCC2) and the functoriality of $\times$, therefore holds in $\mathcal C[x]$ as well, then if we apply $\iota_x$ to both sides we get:
...
of results, each a stronger version of the previous one. These results go under the name of \textit{deduction theorem} or, more broadly, \textit{functional completeness} theorems.

\begin{lemma}[Deduction lemma for formal polynomial terms]
    Given a positive intuitionistic propositional calculus $\mathcal P$, an object $A$ of $\mathcal P$ and an indeterminate arrow $x : \top \to A$, let
    $\mathcal P\langle x\rangle$ be the formal polynomial term calculus introduced in the following remark. For every well-typed formal arrow term
    $\varphi(x) : B \to C$ of $\mathcal P\langle x\rangle$, there is a canonically associated arrow
    \[ \kappa_{x \in A}\varphi(x) : A \land B \to C \]
    in $\mathcal P$.
\end{lemma}

\begin{remark}[Formal polynomial term calculus]
    ...
    $\mathcal P\langle x\rangle$ the \textit{formal polynomial term calculus over $\mathcal P$}. Its objects, tautological object, conjunction and implication are those of
    $\mathcal P$, while its arrows are the well-typed formal terms generated by the grammar
    \[ \varphi,\psi ::= h \mid x \mid \psi \circ \varphi \mid \langle \varphi,\psi\rangle \mid \varphi^*, \]
    with composition, pairing and currying given by the corresponding term constructors.
    Here $h \in \mor(\mathcal P)$ is regarded as an atomic term with the same source and target as the corresponding arrow of $\mathcal P$. In particular, the distinguished identity, terminal,
    projection and evaluation arrows $1_B$, $!_B$, $\pi_{B,C}$, $\pi_{B,C}'$ and $\textnormal{ev}_{B,C}$ of $\mathcal P$ are regarded as atomic terms.
    No equivalence relation is imposed at this stage: for example, if $g \circ f = h$ in $\mathcal P$, then the atomic term $h$ and the formal composite $g \circ f$
    are still distinct terms of $\mathcal P\langle x\rangle$.

    When a cartesian closed category $\mathcal C$ is regarded as a positive intuitionistic propositional calculus and $x : 1 \to A$, there is a canonical surjection from
    the formal arrow terms of $\mathcal C\langle x\rangle$ onto the arrows of $\mathcal C[x]$: it sends $h$ to $[h]$, $x$ to $[x]$ and commutes with composition,
    pairing and currying. Thus, $\mathcal C[x]$ is canonically isomorphic to the quotient of $\mathcal C\langle x\rangle$ by the kernel equivalence relation of this map.
\end{remark}

\begin{remark}[Other versions of the deduction theorem]
    The lemma above can also be stated and proved for conjunction calculi, intuitionistic propositional calculi and cartesian closed categories, using the corresponding formal polynomial term calculi.
    The idea of the proof is the same as the one we are about to show, with the appropriate cases added to or removed from the induction. We prove the lemma for a positive intuitionistic propositional calculus
    because this is the syntactic construction needed for the next result on cartesian closed categories.
\end{remark}

\begin{proof}
    By the definition of $\mathcal P\langle x\rangle$, every formal arrow term $\varphi(x) : B \to C$ has one of the following forms:
    \[ g,\quad x,\quad \psi(x) \circ \chi(x),\quad \langle \psi(x),\chi(x)\rangle,\quad \psi(x)^*, \]
    where $g \in \mor(\mathcal P)$ and $x : \top \to A$ is the indeterminate arrow. We proceed by induction on the \textit{length} of the term, defined as follows:
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        where $\alpha_{A,B,C} : (A \land B) \land C \to A \land (B \land C)$ denotes the associativity arrow.
    \end{itemize}
    ...
    [AGGIUNGERE REFERENZA SU QUESTA MAPPA]
\end{proof}

\begin{notation}
    We write $\kappa_{x \in A}\varphi(x)$ for the arrow associated by the lemma to a formal term $\varphi(x)$ of $\mathcal P\langle x\rangle$. The subscript $x \in A$ indicates that $x$ is of
    \textit{type} $A$, i.e.,\ $x : \top \to A$. At this stage, $\kappa_{x \in A}$ is defined on formal terms, not on their equivalence classes in a quotient.
\end{notation}

In a cartesian closed category, the lemma can be applied to a representative of a polynomial arrow. The following theorem adds the defining equation and uniqueness; in particular,
uniqueness makes the resulting arrow independent of the chosen representative.
