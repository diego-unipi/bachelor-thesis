...
\begin{definition}[Conjunction calculus]
    ...
    called \textit{conjunction}. The following rules, which state the existence of certain classes of arrows, also hold:
    \begin{itemize}
...
(Conj 1) as the \textit{introduction rule} for $\top$ together with the \textit{weakening rule}. We will progressively add more rules, or rather more arrows, to our deductive systems until we reach a complete intuitionistic deductive system.
...
\begin{remark}
    As with the conjunction rules, we can see an analogy with natural deduction here. In fact, (Impl 1) is the \textit{elimination rule} for implication; however, (Impl 2) does not correspond to a basic inference rule, but rather to a derived one:
...
\begin{definition}[Positive intuitionistic calculus freely generated by a graph]
    Given a graph $\mathcal G$, the \textit{positive intuitionistic propositional calculus freely generated by $\mathcal G$}, written as $\mathcal P(\mathcal G)$,
...
\begin{notation}
    $S[a/x]$ can regarded as the process of \textit{substituting} the arrow $a$ for the indeterminate arrow $x$, therefore we will use the notation:
