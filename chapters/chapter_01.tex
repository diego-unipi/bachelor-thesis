...
\begin{definition}
    ...
    as the equivalence class of $f$ in $\mathcal F(\mathcal G)$, we're just forgetting the additional structure properties given by the cartesian closedness.
\end{definition}
...
\begin{proof}
    We need to uniquely define a cartesian closed functor $\overline F : \mathcal F(\mathcal G) \to \mathcal C$, we know that the objects of $\mathcal F(\mathcal G)$ coincides with the objects of the positive intuitionistic calculus $\mathcal P(\mathcal G)$,
    ...
    and since $\mathcal U(\overline F)$ coincides with $\overline F$ on objects, because of the definition, we have that $\overline F(X) = F(X)$, therefore $\overline F$ is uniquely defined and determined on the objects thanks to its closed cartesian structure\footnote{To be completely rigorous, we need to invoke the
    principles of recursion on the grammar, for the definition of $\overline F$, and induction on the grammar, for the proof of uniqueness.}.\\
    By the same reasoning, we can uniquely define $\overline F$ on the arrows of $\mathcal P(\mathcal G)$ (the positive intuitionistic calculus generated by $\mathcal G$), since they are generated by the grammar:
    \[ f,g ::= h \mid 1_A \mid !_A \mid f \circ g \mid \pi_{A,B} \mid \pi_{A,B}' \mid \langle f,g\rangle \mid f^* \mid \ev_{A,B},\]
    where $h \in \mor(\mathcal G)$, $A$ and $B$ are objects of $\mathcal F(\mathcal G)$, and $f, g \in \mor(\mathcal F(\mathcal G))$; then we can 
...
\begin{remark}[$\mathcal F$ is a functor]
    From the universal property just proved we can easily deduce that $\mathcal F : \mathsf{Grph} \to \mathsf{CCC}$ is a functor. We already know how it associates a cartesian closed category $\mathcal F(\mathcal G)$ to each graph $\mathcal G$, 
...
\begin{remark}
    ...
    so that the functor freely generated CCC, $\mathcal F : \mathsf{Grph} \to \mathsf{CCC}$, is left adjoint to the forgetful functor $\mathcal U : \mathsf{CCC} \to \mathsf{Grph}$, and vice versa (so we can write $\mathcal F \dashv \mathcal U$).
\end{remark}
...
\begin{definition}[Polynomial category - v.1]
    Given a (cartesian, cartesian closed) category $\mathcal C$, two objects of such category $A_0$ and $A$, and an indeterminate arrow $x : A_0 \to A$, we define the \textit{polynomial category} $\mathcal C[x]$ 
    as the cartesian closed category freely generated by the graph $\mathcal U(\mathcal C)$ with the additional arrow $x : A_0 \to A$.
\end{definition}

\begin{notation}[Polynomial arrows]
    We will denote with $\varphi(x), \psi(x), \ldots$ the arrows of $\mathcal C[x]$ that are generated by the indeterminate arrow $x : A_0 \to A$. These arrows 
    can be thought as proofs under the \textit{assumption} $x : A_0 \to A$.
\end{notation}

Another way to define the polynomial category is following a similar approach to the one used to define the freely generated cartesian closed category by a graph.

\begin{definition}[Polynomial category - v.2]
    Given a (cartesian, cartesian closed) category $\mathcal C$, two objects of such category $A_0$ and $A$, and an indeterminate arrow $x : A_0 \to A$, we define the \textit{polynomial category} $\mathcal C[x]$
    in two steps:
    \begin{itemize}
        \item first we define the positive intuitionistic propositional calculus generated by the graph $\mathcal U(\mathcal C)$ with the additional arrow $x : A_0 \to A$, whose formulas are generated by the grammar:
        \[ A,B ::= \top \mid X \mid A \times B \mid A^B, \]
        with $X \in \ob(\mathcal C)$, and whose proofs are generated by the grammar:
        \[ f,g ::= h \mid x \mid 1_A \mid !_A \mid f \circ g \mid \pi_{A,B} \mid \pi_{A,B}' \mid \langle f,g\rangle \mid f^* \mid \ev_{A,B},\]
        with $A, B \in \ob(\mathcal C)$ and $h \in \mor(\mathcal C)$;
        ...
        \[ \iota_x : \mathcal C \hookrightarrow \mathcal C[x] : X \mapsto X, f \mapsto [f], \]
        is a (cartesian, cartesian closed) functor. We choose the smallest equivalence relation with these properties, and we denote it with $\underset{x}{=}$. In the case of a cartesian closed category, we need the following properties for $\underset{x}{=}$ to be satisfied:
        \begin{itemize}[label=$\ast$]
            ...
            \item if $\varphi(x) \underset{x}{=} \varphi'(x)$ and $\psi(x) \underset{x}{=} \psi'(x)$ then $\varphi(x) \circ \psi(x) \underset{x}{=} \varphi'(x) \circ \psi'(x)$ (the equivalence relation is compatible with composition);
            \item $1_B \circ \varphi(x) \underset{x}{=} \varphi(x) 1_A \underset{x}{=} \varphi(x)$ (the identity laws);
            \item $(\varphi(x) \circ \psi(x)) \circ \chi(x) \underset{x}{=} \varphi(x) \circ (\psi(x) \circ \chi(x))$ (the associativity law).
            ...
            \item for each $\varphi(x) \underset{x}{=} \varphi'(x)$ and $\psi(x) \underset{x}{=} \psi'(x)$, we have $\langle \varphi(x), \psi(x)\rangle \underset{x}{=} \langle \varphi'(x), \psi'(x)\rangle$ (the pairing is compatible with the equivalence relation);
            \item if $f^* = h$, then $f^* \underset{x}{=} h$ (the currying in $\mathcal C[x]$ extends the one in $\mathcal C$);
            \item for each $\varphi(x) \underset{x}{=} \varphi'(x)$, we have $\varphi(x)^* \underset{x}{=} \varphi'(x)^*$ (the currying is compatible with the equivalence relation);
            \item for every object $A$ of $\mathcal C$, $1_A^{\mathcal C} \underset{x}{=} 1_A^{\mathcal C[x]}$ (so the identities are preserved).
        \end{itemize}
        The fact that $!_A^{\mathcal C} \underset{x}{=} !_A^{\mathcal C[x]}$ follows from the (CC1) in $\mathcal C[x]$; the fact that $\pi_{A,B}^{\mathcal C} \underset{x}{=} \pi_{A,B}^{\mathcal C[x]}$ follows from (CC2a) + the fact that $1_A^{\mathcal C} \underset{x}{=} 1_A^{\mathcal C[x]}$ and the preservation of pairing:
        \[ \langle \pi_{A,B}^{\mathcal C}, (\pi_{A,B}')^{\mathcal C}\rangle = 1_{A \times B}^{\mathcal C} \implies \langle \iota_x(\pi_{A,B}^{\mathcal C}), \iota_x((\pi_{A,B}')^{\mathcal C})\rangle \underset{x}{=} 1_{A \times B}^{\mathcal C[x]}, \]
        now composing with $\pi_{A,B}^{\mathcal C[x]}$ we get $\iota_x(\pi_{A,B}^{\mathcal C}) \underset{x}{=} \pi_{A,B}^{\mathcal C[x]}$, same exact argument for $(\pi_{A,B}')^{\mathcal C}$. Lastly, the fact that $\ev_{A,B}^{\mathcal C} \underset{x}{=} \ev_{A,B}^{\mathcal C[x]}$
        follows from (CCC2) taking $k = 1_{A^B}^{\mathcal C}$:
        \[ (\ev_{A,B}^{\mathcal C} \circ (1_{A^B}^{\mathcal C} \times 1_B^{\mathcal C}))^* = 1_{A^B}^{\mathcal C} \implies (\ev_{A,B}^{\mathcal C})^* = 1_{A^B}^{\mathcal C}, \]
        this identity is derived just by (CCC2) and the functoriality of $\times$, therefore holds in $\mathcal C[x]$ as well, then if we apply $\iota_x$ to both sides we get:
        \[ (\iota_x(\ev_{A,B}^{\mathcal C}))^* \underset{x}{=} 1_{A^B}^{\mathcal C[x]} \underset{x}{=} (\ev_{A,B}^{\mathcal C[x]})^*, \]
        applying (CCC1) to both sides we get $\iota_x(\ev_{A,B}^{\mathcal C}) \underset{x}{=} \ev_{A,B}^{\mathcal C[x]}$.
    \end{itemize}
\end{definition}
...
\begin{remark}[Well-posedness of the definition v.2]
    ...
    One can easily check that this equivalence relation the remaining properties above, therefore the quotient is a (cartesian, cartesian closed) category and the immersion $\iota_x : \mathcal C \hookrightarrow \mathcal C[x]$ is a (cartesian, cartesian closed) functor,
    so the definition above is well-posed.
\end{remark}

\begin{notation}[Immersion of $\mathcal C$ into $\mathcal C{[x]}$]
    From now on we will denote with $\iota_x : \mathcal C \hookrightarrow \mathcal C[x]$ the immersion of $\mathcal C$ into $\mathcal C[x]$, which is a (cartesian, cartesian closed) functor.
    When it is clear from the context, we will sometimes omit the subscript $x$ and just write $\iota : \mathcal C \hookrightarrow \mathcal C[x]$.
\end{notation}
...
\begin{theorem}[Universal property of polynomial categories]
    Given a (cartesian, cartesian closed) category $\mathcal C$, two objects of such category $A_0$ and $A$, an indeterminate arrow $x : A_0 \to A$, a (cartesian, cartesian closed) category $\mathcal D$, a functor $F : \mathcal C \to \mathcal D$ and an arrow $a : F(A_0) \to F(A)$ in $\mathcal D$,
    there is a unique (cartesian, cartesian closed) functor $\widehat F : \mathcal C[x] \to \mathcal D$ such that the following diagram commutes:
...
\begin{proof}
    ...
    \end{align*}
    If we check that $\widehat F$ is well-defined on the equivalence classes of arrows, then it will be automatically a cartesian closed functor, it will make the diagram commute, and it will be uniquely determined by the definition above\footnote{As we remarked before, to be completely rigorous, we should invoke recursion and induction on the grammar.}.\par
    The idea for proving the well-definedness is the same as in the proof of the other universal property already seen, we define the equivalence relation on the arrows $f \sim_{\widehat F} g$ if and only if $\widehat F(f) = \widehat F(g)$, and we prove that such relation respects all the properties listed in the definition,
    so it must contain $\underset{x}{=}$, and this will imply that if $[f] = [g]$ then $\widehat F(f) = \widehat F(g)$. All the checks are similar (?) to the ones in the other proof, so we omit them here. 
\end{proof}
