...
\begin{proof}
    ...
    where $(1_A \times s) \circ \sigma_{N,A} = \sigma_{N,A} \circ (s \times 1_A)$ follows form the definition of $\sigma_{N,A}$, and $h' := \Phi(h \circ \sigma_{N,A})$. The RHS becomes
...
\begin{definition}[$\lambda$-\textsf{Calc}]
    We define by $\lambda$-\textsf{Calc} the category of \emph{typed $\lambda$-calculi}, whose objects are typed $\lambda$-calculi that respect the definition above, possibly with additional basic types, terms, type-forming rules, term-forming rules and equations, and whose morphisms are translations between them.
...
\begin{example}[Internal language of a CCC]
    ...
    \begin{itemize}
        ...
        where $\langle a, b \rangle$ is the pairing arrow, $f^\sharp(a) := \ev_{A,B}(f,a) \circ \langle \pi_{B^A,A} ,a \rangle$ is the evaluation arrow, and $\lambda_{x \in A} \varphi(x) := \ulcorner \kappa_{x \in A}(\varphi(x)) \circ \langle 1_A, !_A \rangle \urcorner$,
        just like we did in the proof of Lambek's normal form, \Cref{cor:lambeks-normal-form}. We define $* = !_1$. Furthermore, if $\varphi(x)$ and $\psi(y)$ are terms of type $A$, whose free variables belongs to $X$, $\varphi(x) \underset{X}{=} \psi(y)$
        is said to hold in $\mathbf L(\mathcal C)$ if the corresponding polynomial arrows are equal, $\underset{X}{=}$, in $\mathcal C[x_1,\dots,x_n]$.
        \item the equations are the standard ones, and one can verify that they hold in $\mathbf L(\mathcal C)$, since they hold in the underlying cartesian closed structure.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item on the equations, $\mathbf L(F)$ preserves them, since it the definition is well-posed, the functor $\widehat{F}_X$ is cartesian closed and, it is easy to verify, it preserves the chosen WNNOs and iterators, thus preservers the equalities between terms.
    \end{itemize}
\end{example}

\begin{notation}[Multi-index notation for indeterminates]
    We will sometimes abbreviate the indeterminates of a term $\varphi(x_1,\dots,x_n) : 1 \to A$ as $\varphi(X)$, using $X = x_1,\dots,x_n$. Furthermore, we will use the multi-index notation also for the subscript of the natural inclusion functor $\iota_X : \mathcal C \to \mathcal C[X]$, and for the the polynomial category itself, $\mathcal C[X]$.
\end{notation}
slob
...
\begin{definition}[CCC defined by a $\lambda$-calculus]
    ...
    \begin{itemize}
        ...
        Equivalently, $\Hom_{\mathbf C(\mathcal L)}(A,B) = \{[(x,\varphi(x))] : x \in A, \varphi(x) \in B\}$, where $[(x,\varphi(x))]$ is the equivalence class of $(x,\varphi(x))$ with respect to the relation
        \[ (x,\varphi(x)) \sim (y,\psi(y)) \quad\overset{\textnormal{def}}{\iff}\quad \varphi(z) \underset{\{z\}}{=} \psi(z), \]
        for $x,y \in A$, $\varphi(x),\psi(y) \in B$ and $z \in A$ (basically, up to $\alpha$-conversion, they are the same term). The categorical of $\mathbf C(\mathcal L)$ can be retrieved from the type-forming rules, as follows:
        \begin{itemize}[label=$*$]
        ...
        \begin{itemize}[label=$*$]
            ...
            \item $\pi_{A,B} = [(z,\pi_{A,B}(z))] : A \times B \to A$ and $\pi_{A,B}' = [(z,\pi_{A,B}'(z))] : A \times B \to B$ are the projections;
            \item $\langle [(x,\varphi(x))],[(y,\psi(y))] \rangle = [(z,\langle \varphi(z),\psi(z) \rangle)] : C \to A \times B$ is the pairing of two arrows $[(x,\varphi(x))]$ and $[(y,\psi(y))]$;
            \item $[(z,\chi(z))]^* = [(x,\lambda_{y \in B} \chi(\langle x,y\rangle)] : A \to B$, where $z \in A \times B^C$ is the currying of an arrow $[(z,\chi(z))] : A \times B \to C$;
            \item $\ev_{C,A} = [(y,\ev_{C,A}(\pi_{B^C,A}(y),\pi_{A,B^C}(y)))] : A \times B^C \to B$ is the evaluation arrow.
        \end{itemize}
        Finally, the chosen WNNO and iterator are defined as:
        \begin{itemize}[label=$*$]
            \item $0 = [(x,0)] : 1 \to N$;
            \item $s = [(x,s(x))] : N \to N$;
            \item $I_A = [(w,I_B(\pi_{B,B^B}(\pi_{B \times B^B,N}(w)), \pi_{B,B^B}'(\pi_{B \times B^B,N}(w)), \pi_{B \times B^B,N}(w)))] : (B \times B^B) \times N \to B$.
        \end{itemize}
    \end{itemize}
    ...
    \[ \mathbf C(\Phi) := \begin{cases}
        A \mapsto \Phi(A) \\
        [f \in B^A] = [(x,\varphi(x))] \mapsto [\Phi(f) \in \Phi(B)^{\Phi(A)}] = [(\Phi(x),\Phi(\varphi(x)))]
    \end{cases}, \]
    and easily follows form the definition of translation that $\mathbf C(\Phi)$ is well-defined and preserves the cartesian closed structure, the chosen WNNOs and iterators.
\end{definition}
...
\begin{proof}
    ...
    First, we need to define a natural map $\iota_x : \mathbf C(\mathcal L) \to \mathbf C(\mathcal L(x))$.
\end{proof}

Finally we can prove the equivalence of categories between $\mathsf{CCC}_N$ and $\lambda$-\textsf{Calc}.
