...
\begin{frame}{Punto fisso nel caso diagonale}
  ...
  \begin{tikzpicture}[node distance=7mm]
    ...
    \node[accent,right=of code,text width=40mm,minimum height=21mm] (self)
      {\textbf{3. Applicare la \mbox{funzione} al suo codice}\\[3pt]
       $s:=\phi(\ulcorner d \urcorner,\ulcorner d \urcorner)$};}
  ...
  \uncover<6>{
  \[
    s=\phi(\ulcorner d \urcorner,\ulcorner d \urcorner)=d(\ulcorner d \urcorner)=f\bigl(\phi(\ulcorner d \urcorner,\ulcorner d \urcorner)\bigr)=f(s)
  \]
  }
\end{frame}
...
\begin{frame}<1-2>[t]{One fixed-point theorem to rule them all}
  ...
  \vspace{0mm}
  \begin{block}{Teorema di punto fisso (Lawvere, 1969)}
    \vspace{1mm}
