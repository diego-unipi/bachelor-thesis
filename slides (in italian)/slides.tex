...
\begin{frame}{Applicazioni di Lawvere}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\centering\arraybackslash}p{.13\textwidth} >{\bfseries}L{.18\textwidth} Y L{.215\textwidth}@{}}
    ...
    \multirow{5}{*}[-15mm]{\textcolor{maincolor}{$\RecTot$}}
      & Halting problem
      & L'haling set $H_1 = \{ (\alpha, x) \in \NN^2 | u_1(\alpha, x) \ne \bot \}$ non è decidibile.
      & Non esiste un algoritmo che decide se un programma termina. \\
    & Funzioni ricorsive totali
      & Non esiste un'enumerazione uniforme di tutte le funzioni ricorsive totali $\NN\to\NN$.
      & Sono numerabili, ma non elencabili da un unico algoritmo totale. \\
    & Insiemi decidibili
      & Non esiste un'enumerazione uniforme di tutti gli insiemi decidibili $A\subseteq\NN$.
      & Sono numerabili, ma non elencabili uniformemente. \\
    & Insiemi semidecidibili
      & Esiste un insieme semidecidibile $A\subseteq\NN$ con $A^{\complement}$ non semidecidibile.
      & Non sono chiusi per complemento. \\
    & Aritmetica vera
      & $\mathrm{Th}(\NN;L_{\textnormal{arit}})$ non è ricorsivamente enumerabile.
      & Le verità aritmetiche non sono enumerabili da un algoritmo. \\
    \addlinespace[.7mm]
    \textcolor{maincolor}{$\mathsf{Set}$}
      & Linguaggi su $\{1\}$
      & Esiste un linguaggio $L\subseteq\{1\}^*$ non r.e.
      & Anche un alfabeto unario ammette linguaggi non r.e. \\
    \bottomrule
  \end{tabularx}
\end{frame}
