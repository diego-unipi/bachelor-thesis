...
\begin{frame}{Applicazioni di Lawvere}
  ...
  \makebox[\textwidth][c]{\begin{tabularx}{\dimexpr\textwidth+12mm\relax}{@{}>{\centering\arraybackslash}p{.10\textwidth} >{\bfseries}L{.19\textwidth} Y L{.27\textwidth}@{}}
    ...
    & Insiemi semidecidibili
      ...
      & Saper semi-decidere un insieme non significa saper semi-decidere il suo complementare. \\
...
\begin{frame}<1-3>{Vediamo un esempio in dettaglio}
  \framesubtitle{L'halting problem in $\RecTot$}
  \small
  Supponiamo di avere un algoritmo che decide se un programma termina su un dato input.

  \vspace{2mm}
  \centering
  \begin{tikzpicture}[node distance=3mm,
      haltingstep/.style={text width=128mm,inner xsep=4pt,inner ysep=3pt,font=\small}]
    \only<2>{\tikzset{node distance=4.5mm}}
    \only<3>{\tikzset{node distance=3.5mm}}
    \node[categoryaccent,haltingstep] (total)
      {\textcolor{maincolor}{\textbf{1. Il decisore darebbe ci permetterebbe di definire una funzione calcolabile totale}}\\[2pt]
    ...
    \uncover<2->{
      \node[accent,haltingstep,below=of total] (universal)
        {\textcolor{maincolor}{\textbf{2. $F$ è una \textit{mappa universale}}}\\[2pt]
    ...
    \uncover<3->{
      \node[warning,haltingstep,below=of universal] (contradiction)
        {\textcolor{maincolor}{\textbf{3. Dunque vale il teorema di Lawvere}}\\[2pt]
         \mbox{Quindi ogni funzione calcolabile totale da $\NN\to\NN$ avrebbe un punto fisso, ma}\\[2pt]
         \mbox{$s:\NN\to\NN:n\mapsto n+1$}\\[2pt]
         {\settowidth{\dimen0}{Quindi ogni funzione calcolabile totale da $\NN\to\NN$ avrebbe un punto fisso, ma}%
  ...
  \begin{tikzpicture}[
    ...
    \node[semanticroute,text width=25mm,minimum height=19mm,font=\bfseries]
      ...
      {$\mathsf{Truth}$ non è definibile in $T$\\[.4mm]
       {\scriptsize\mdseries\spaceskip=0pt\xspaceskip=0pt Corollario immediato\\[-.2mm]}};
    ...
    \draw[semanticflow] (tarski.east) -- ([yshift=3mm]godel.west);
