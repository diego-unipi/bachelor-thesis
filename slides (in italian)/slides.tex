...
\begin{frame}{Teoremi di punto fisso}
  ...
  \begin{tikzpicture}[node distance=5mm and 5mm]
    ...
    \node[accent,below right=of fixed,text width=40mm,align=center] (diagonal)
      {Diagonali: si dimostrano\\tramite l'autoreferenzialità};
...
\begin{frame}{Punto fisso nel caso diagonale}
  ...
  \begin{tikzpicture}
    \node[rounded corners=3pt,draw=maincolor,line width=1pt,fill=white,
      ...
      {Tipicamente, nei casi diagonali, si ha una mappa $\phi:A\times A\to B$, detta \textcolor{maincolor}{\textbf{codifica universale}} (o semplicemente codifica), tale che:
      \[ \forall d : A \to B \quad \exists \ulcorner d \urcorner \in A \quad \text{ per cui } \quad\phi(\ulcorner d \urcorner,-)=d(-) \]\\[1mm]};
  \end{tikzpicture}

  \vspace{2mm}
  \uncover<2->{Dato $f : B \to B$ possiamo trovare un punto fisso nel modo seguente:}\par
  \vspace{2mm}
  \begin{tikzpicture}[node distance=7mm]
    ...
    \node[accent,right=of diag,text width=32mm,minimum height=21mm] (code)
      {\textbf{2. \mbox{Prenderne} il codice}\\[3pt]
       $\phi(\ulcorner d \urcorner,-)=d(-)$};}
    \only<5->{
    \node[accent,right=of code,text width=40mm,minimum height=21mm] (self)
      {\textbf{3. Applicare la \mbox{funzione} al suo codice}\\[3pt]
       $s:=d(\ulcorner d \urcorner)$};}
    \only<4->{\draw[flowaccent] (diag) -- (code);}
    \only<5->{\draw[flowaccent] (code) -- (self);}
  \end{tikzpicture}

  \vspace{-2mm}
  \uncover<6>{
  \[
    s = d(\ulcorner d \urcorner)=f\bigl(\phi(\ulcorner d \urcorner,\ulcorner d \urcorner)\bigr)= f(d(\ulcorner d \urcorner)) = f(s).
  \]
  }
\end{frame}
...
\begin{frame}{Dove formalizzare il principio diagonale}
  ...
  \begin{block}{\textbf{Categoria cartesiana}}
    Una \textbf{categoria cartesiana} è una categoria che ammette:
    \begin{itemize}
  ...
  \begin{block}{\textbf{Categoria cartesiana chiusa}}
    ...
    \begin{itemize}
      \item un esponenziale $B^A$ per ogni coppia di oggetti $A,B$, con la
        ...
        \[
          \Hom(X\times A,B)\overset{1:1}{\longleftrightarrow}\Hom(X,B^A).
        \]
    \end{itemize}
  \end{block}
\end{frame}

\begin{frame}<1-2>[t]{One fixed-point theorem to rule them all}
  \framesubtitle{Il teorema di punto fisso di Lawvere}
  \small
  Lawvere generalizza il principio precedente: una codifica universale produce sempre
  punti fissi.

  \vspace{0mm}
  \begin{block}{Teorema (Lawvere, 1969)}
    \vspace{1mm}
    \begin{columns}[T,onlytextwidth]
      \begin{column}{.47\textwidth}
        ...
        è debolmente surgettiva sui punti,
      \end{column}
      \begin{column}{.47\textwidth}
        \centering
        \textcolor{maincolor}{\textbf{Forma cartesiana}}\par\smallskip
  ...
  \((-)^*\colon\!\Hom(A\times A,B)
    \overset{\cong}{\longrightarrow}\Hom(A,B^A), \quad
    (-)^\sharp\colon\!\Hom(A,B^A)
    \overset{\cong}{\longrightarrow}\Hom(A\times A,B).\)
  \vspace{-1mm}
  }
\end{frame}
...
\begin{frame}{Curry--Howard--Lambek}
  ...
  \begin{block}{Equivalenza di categorie}
    ...
    \begin{tabularx}{.97\textwidth}{@{}p{15mm} >{\raggedright\arraybackslash}X c p{15mm} >{\raggedright\arraybackslash}X@{}}
      \multicolumn{2}{c}{\color{maincolor}\bfseries $\mathsf{CCC}$}
        & &
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{.47\textwidth}
      ...
      \begin{block}{Teoremi di esistenza}
        Se esiste una codifica $\phi$ di uno dei due tipi visti sopra,
    ...
    \begin{column}{.47\textwidth}
      ...
      \begin{block}{Teoremi di impossibilità}
        Se un oggetto $B$ ammette un endomorfismo senza punti fissi, allora non può esistere una codifica:
        \[
...
\begin{frame}{Applicazioni di Lawvere}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\centering\arraybackslash}p{.09\textwidth} >{\bfseries}L{.14\textwidth} L{.24\textwidth} Y@{}}
    ...
    \textcolor{maincolor}{$k\mathsf{Top}$}
      ...
      & Nessuna suriezione continua $I\twoheadrightarrow X^I = C^0(I,X)$;
...
\begin{frame}<1-4>{Vediamo un esempio in dettaglio}
  \framesubtitle{Il teorema di Cantor in $\mathsf{Set}$}
  \small
  \begin{block}{Teorema (Cantor, 1891)}
    \centering
    Per ogni insieme $A$, si ha che $|A|<|2^A|$.
  \end{block}
  ...
  \begin{tikzpicture}[node distance=5mm,
      cantorstep/.style={text width=128mm,inner xsep=4pt,inner ysep=3pt,font=\small}]
    \node[categoryaccent,cantorstep] (surjection)
      {\mbox{Supponiamo per assurdo che $|A| \geq 2^{|A|}$.}};

    \uncover<3->{
      \node[accent,cantorstep,below=of surjection] (universal)
        {\mbox{Allora esiste una mappa surgettiva $f : A \twoheadrightarrow 2^A$.}};
      \draw[flow] (surjection) -- (universal);
    }

    \uncover<4->{
      \node[warning,cantorstep,below=of universal] (contradiction)
        {\begin{minipage}{\linewidth}
         \raggedright
         \mbox{Dunque si applica Lawvere, per cui ogni endomorfismo di $2$ deve avere un punto fisso,}\\[1pt]
         in particolare:\\[1pt]
         \makebox[\linewidth][c]{$\neg : 2 = \{0,1\} \to 2$}\\[1pt]
         deve averlo, assurdo.
         \end{minipage}};
...
\begin{frame}{Applicazioni di Lawvere}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\centering\arraybackslash}p{.13\textwidth} >{\bfseries}L{.18\textwidth} Y L{.215\textwidth}@{}}
    ...
    & Teorema di Rogers
      & Se $\varphi:\NN\to\NN$ è calcolabile totale, allora esiste $n\in\NN$
        con $u_1(\varphi(n),-)=u_1(n,-)$.
      & Ogni algoritmo che termina sempre manda il codice di almeno un programma in un altro codice di un programma equivalente. \\
    \addlinespace[.7mm]
...
\begin{frame}{Applicazioni di Lawvere}
  ...
  \makebox[\textwidth][c]{\begin{tabularx}{\dimexpr\textwidth+12mm\relax}{@{}>{\centering\arraybackslash}p{.10\textwidth} >{\bfseries}L{.19\textwidth} Y L{.27\textwidth}@{}}
    ...
    \multirow{5}{*}[-15mm]{\textcolor{maincolor}{$\RecTot$}}
      & Halting problem
      & L'halting set $H_1 = \{ (e, x) \in \NN^2 | u_1(e, x) \ne \bot \}$ non è decidibile.
    ...
    & Insiemi semidecidibili
      ...
      & Saper semidecidere un insieme non significa saper semidecidere il suo complementare. \\
    ...
    \uncover<3->{
      \node[accent,haltingstep,below=of total] (universal)
        {Per ogni funzione calcolabile totale $g : \NN \to \NN$, esiste un suo \textit{codice} $\ulcorner g \urcorner$ tale che\\[2pt]
         \mbox{$\phi(\ulcorner g \urcorner,x)=g(x)\quad\forall x\in\NN$}.};
