...
This template is based on \hrefcol{https://www.overleaf.com/latex/templates/sintef-presentation/jhbhdffczpnx}{SINTEF Presentation} from \hrefcol{mailto:federico.zenith@sintef.no}{Federico Zenith} and its derivation \hrefcol{https://github.com/TOB-KNPOB/Beamer-LaTeX-Themes}{Beamer-LaTeX-Themes} from Liu Qilong and Andrea Gasparini, and \hrefcol{https://github.com/MattiaIppoliti}{Mattia Ippoliti}. 
...
\begin{frame}{Beamer for \texttt{dmpisa} slides}
\begin{itemize}
...
\framesubtitle{Selecting the \texttt{dmpisa} Theme}
To start working with \texttt{dmpisa}, start a \LaTeX\ document with the
preamble:
\begin{block}{Minimum \texttt{dmpisa} Beamer Document}
\verb|\documentclass{beamer}|\\
\verb|\usetheme{dmpisa}|\\
\verb|\begin{document}|\\
...
By default, some text such as ``Theorem", ``Definition", as well as the format of the date, is in English. To change to your language (say, Italian), you must add \texttt{[italian]} in the document class, and use the package \texttt{babel}, as follows:

\begin{block}{}
\verb|\documentclass[italian]{beamer}|\\
...
The header of this theorem is in English
\end{theorem}
\end{column}
\begin{column}{0.45\textwidth}

%% COMMENT FOR ANYONE WITH THE INTENTION OF COPYING THIS PIECE OF CODE
% Despite rendering as a theorem would render, this is actually an hardcoded blocks
% The reason for this is that I don't think there's any way to have BOTH "Theorem"
% and "Teorema", and so the Italian version must be faked only for the purpose of this
% guide. If you intend to use a theorem and you want it in Italian, use a normal
% "\begin{theorem}" and make sure to set "[italian]" in the document class and to use
% the package "babel"
\begin{block}{Teorema (Weierstrass)}
The header of this theorem is in Italian
\end{block}
\end{column}
\end{columns}
\vspace{\baselineskip}
Note that the ``Table of Contents" won't be translated by this since it is hardcoded in the style file, and must be manually modified
\end{frame}

\begin{frame}[fragile]{Theorem appearance}
\begin{columns}
\begin{column}{0.45\textwidth}
By default, theorems and definitions appear as follows:
\end{column}
\begin{column}{0.45\textwidth}
\begin{theorem}[Weierstrass]
    if $f$ is continuous on $[a, b]$, then $f$ must attain a maximum and a minimum, each at least once
\end{theorem}
\end{column}
\end{columns}
\begin{columns}
\begin{column}{0.45\textwidth}
You can compress them and make them look like this by adding \texttt{[headerlesstheorems]} in the \texttt{usetheme} as shown below
\end{column}
\begin{column}{0.45\textwidth}

%% COMMENT FOR ANYONE WITH THE INTENTION OF COPYING THIS PIECE OF CODE
% Despite rendering as a headerless theorem would render, this is actually an hardcoded blocks
% The reason for this is that it's not possible to have BOTH the headerfull and headerless
% versions of theorems, and so the headerless version must be faked only for the purpose of this
% guide. If you intend to use a headerless theorem, use a normal "\begin{theorem}" and make sure
% to set "[headerlesstheorems]" in the theme
\begin{block}{}
    \hspace{2pt}\textcolor{maincolor}{\textit{\textbf{Theorem 2 (Weierstrass):}}} \hspace{1pt}
    if $f$ is continuous on $[a, b]$, then $f$ must attain a maximum and a minimum, each at least once
\end{block}
\end{column}
\end{columns}
...
\vspace{\baselineskip}
\begin{block}{}
\verb|\usetheme[headerlesstheorems]{dmpisa}|\\
\end{block}
\end{frame}
...
\begin{sidepic}{assets/background_alternative_side}{Side-Picture Slides}
\begin{itemize}
...
\hrefcol{mailto:fabio.durastante@unipit.it}{send them to me!}
\end{itemize}
\end{frame}
